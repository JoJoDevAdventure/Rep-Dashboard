const CallsList = ({ calls, onCall, OnView, isDarkMode }) => {
    ...
    const borderColor = isDarkMode ? "border-p1/30" : "border-p1/10";
  
    return (
      <div className={`w-full p-6 h-[75vh] overflow-y-auto border-2 ${borderColor} rounded-lg ${bgColor}`}>
        {/* Table Header */}
        <div
            ...
            >
              ...
              <div className="col-span-3 flex justify-center gap-4">
                <button
                  ...
                  onClick={() => OnView(call)} // Pass the current call to the OnView callback
                >
                  View Listing
                </button>
                <button
                  className={`p-2 rounded ${isDarkMode ? "bg-s3" : "bg-s3"}`}
                  onClick={() => onCall(call)} // Pass the current call to the onCall callback
                >
